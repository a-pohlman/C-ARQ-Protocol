...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (baseMsg == "ReqP2" && negAckMsg != "NACK" && pastBattery-POWER_DRAINED > POWER_DRAINED) {
    ...
    if (baseMsg == "ReqP2" && Serial.available() == 0) {
      pastBattery -= POWER_DRAINED;
      Serial.print("2,N2 ");
      Serial.println(pastBattery);
    }
    ...
    else if (haveP3 == true && negAckMsg == "NACK" && Serial.available() == 0) { 
      pastBattery -= POWER_DRAINED;
      Serial.print("3,N2 ");
      Serial.println(pastBattery);
    } 
