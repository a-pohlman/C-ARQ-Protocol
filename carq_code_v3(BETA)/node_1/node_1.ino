...
#define TROJAN_DELAY 3000 // t0 // Time delay to simulate an infected node with a trojan virus (ms)
...
void loop() {
  ...
  if (Serial.available() > 0) { // if anything is available in the serial then:
    ...
    if (currentBattery > 1000) {currentBattery = 1000;} // check to make sure that we are not going over capacity
    ...
    if (baseMsg == "ReqP1" && negAckMsg != "NACK" && pastBattery-POWER_DRAINED > POWER_DRAINED) { // check we are on the first Req
