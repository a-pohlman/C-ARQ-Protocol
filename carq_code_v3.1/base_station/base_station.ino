...
#define AVAILABLE_NODES 1 // N // The amount of nodes available to transmit to the base station
...
void setup() {
  ...
  Serial.setTimeout(WAIT_TIME); // Set the max amount of time that any read function will take before moving 
...
void loop() {
  ...
  if (cTime - pTime >= WAIT_TIME && !stop) { // if the wait time has been reached run the following:
    ...
    packet = parseMsg(data,0).toInt();
    nodeID = parseMsg(data,1);
    battery = parseMsg(data,2).toFloat();

    // checks whether the packet is correct for the node and if the channel has a success
    if (packet == atNode && randomSuccess <= SUCCESS_THRESHOLD) { // If so
