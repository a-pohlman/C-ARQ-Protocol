...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (baseMsg == "3") {haveP2 = true;}
    else if (baseMsg == "ReqP1" || baseMsg == "ReqP2" || baseMsg == "ReqP4") {haveP2 = false;}


    if (pastBattery-POWER_DRAINED > POWER_DRAINED && negAckMsg == "NACK" && baseMsg != "ReqP4" ) {
      Serial.flush();
    ...
    else if (baseMsg == "ReqP4" && baseMsg != "ReqP1" && baseMsg != "ReqP2" && baseMsg != "ReqP3" && pastBattery-POWER_DRAINED > POWER_DRAINED) {
      Serial.flush();
      while (Serial.available() > 0) {Serial.read();}
